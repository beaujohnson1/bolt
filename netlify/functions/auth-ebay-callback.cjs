...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // CRITICAL FIX: Token exchange must use the ACTUAL redirect URL from eBay settings!
    // Your eBay auth accepted URL: https://easyflip.ai/.netlify/functions/auth-ebay-callback
    let redirectUri;
    if (isProduction) {
      // Use the EXACT URL configured in eBay Developer Console
      redirectUri = 'https://easyflip.ai/.netlify/functions/auth-ebay-callback';
