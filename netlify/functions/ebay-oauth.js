...
async function getAuthUrl(headers, credentials, oauthBase, queryParams, environment = 'production') {
  try {
    ...
    // Use redirect_uri with RuName value for production (THIS WAS WORKING 2 HOURS AGO!)
    if (isProduction) {
      // Use redirect_uri with RuName value - this was the working configuration
      const ruName = 'easyflip.ai-easyflip-easyfl-cnqajybp';
      authUrl.searchParams.append('redirect_uri', ruName);
      console.log('🔗 [EBAY-OAUTH] Using production RuName as redirect_uri (ORIGINAL WORKING CONFIG):', ruName);
    } else {
      // Sandbox uses direct redirect_uri
      authUrl.searchParams.append('redirect_uri', callbackUrl);
      console.log('🔗 [EBAY-OAUTH] Using sandbox redirect_uri:', callbackUrl);
    }
...
async function exchangeCode(headers, credentials, tokenBase, body) {
  try {
    ...
    if (isProduction) {
      // Production uses RuName as redirect_uri value
      const ruName = 'easyflip.ai-easyflip-easyfl-cnqajybp';
      tokenParams.append('redirect_uri', ruName);
      console.log('🔄 [EBAY-OAUTH] Using production RuName as redirect_uri for token exchange:', ruName);
    } else {
      // Sandbox uses redirect_uri
      tokenParams.append('redirect_uri', callbackUrl);
      console.log('🔄 [EBAY-OAUTH] Using sandbox redirect_uri for token exchange:', callbackUrl);
    }
